...
def filter_records_by_columns(df, records):
    """Filter DataFrame by records based on COLUMNS_TO_COMPARE."""
    return df[df[COLUMNS_TO_COMPARE].apply(tuple, axis=1).isin(records)]
...
def sync_records():
    ...
    new_records_df = filter_records_by_columns(update, new_records)
...
def archive_records(current_file, missing_records):
    ...
    """
    records_to_archive = filter_records_by_columns(current_file, missing_records)
    ...
    if not records_to_archive.empty:
        ...
        current_file = current_file[~current_file[COLUMNS_TO_COMPARE].apply(tuple, axis=1).isin(missing_records)]
