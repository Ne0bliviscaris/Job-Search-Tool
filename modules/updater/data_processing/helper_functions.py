...
def convert_k_notation(salary_range):
    ...
    return re.sub(
        r"(\d+(\.\d+)?)k", lambda x: str(int(float(x.group(1)) * 1000)), salary_range
    )
...
def extract_salary_details(cleaned_salary, salary_text):
    ...
    salary_details = salary_text[index + 2 :].strip() if index != -1 else None  # noqa
    # Handle Bulldogjobs and NoFluffJobs case
    if (
        salary_text == "Znamy widełki"
        or "Sprawdź"
        or "Undisclosed salary" in salary_text
    ):
...
def split_salary(processed_salary):
    ...
    if len(salary_parts) >= 2:
        ...
        min_salary = max_salary = (
            int(processed_salary.strip()) if processed_salary else None
        )
...
def remove_remote_status(location: str) -> str:
    ...
    filtered_parts = [
        ...
        if not any(
            kw in part.lower()
            for keywords in remote_work_dict.values()
            for kw in keywords
        )
    ]
